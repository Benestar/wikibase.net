...
COVERALLS_TOKEN=$(cat COVERALLS_TOKEN.env)
...
read -p "Upload coverage report? (Y/n) " RESP
if [ "$RESP" = "Y" ]; then
  packages/coveralls.net.0.6.0/tools/csmacnz.Coveralls.exe --opencover -i coverage.xml --repoToken $COVERALLS_TOKEN --commitId $REPO_COMMIT --commitBranch $REPO_BRANCH --commitAuthor "$REPO_COMMIT_AUTHOR" --commitEmail "$REPO_COMMIT_AUTHOR_EMAIL" --commitMessage "$REPO_COMMIT_MESSAGE" --useRelativePaths
  echo "Coverage report sent"
else
